...
int main() {
	...
	std::vector<glm::vec3> color_map = {glm::vec3(0.f, 0.f, 1.f), glm::vec3(1.f, 0.f, 0.f)};
	float scale = .005;
	float DX = 0;
	float DY = 0;
		...
		*/
		...
		for (int x = 0; x < sim.x_max; x++)
			for (int y = 0; y < sim.y_max; y++) {
				...
				//if (cell.on_boundary) color = glm::vec3(0, 1, 0);
				//if (cell.on_bboundary) color = glm::vec3(1, 1, 0);
